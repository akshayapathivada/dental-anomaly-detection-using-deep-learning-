...
def predict():
    try:
        ...
        # ✅ Store predicted condition in session
        session['predicted_condition'] = predicted_class_label.strip().lower()

        return jsonify({'predicted_class': predicted_class_label, 'probability': f"{probability:.4f}"})
...
def recommendation():
    ...
    if not predicted_condition:
        return render_template('food.html', recommendation="No condition detected. Please upload an image.")
    ...
    # Try using GPT-4, fallback to GPT-3.5 if needed
    model_name = "gpt-4-turbo"  # Change to "gpt-3.5-turbo" if necessary

    try:
        response = openai.ChatCompletion.create(
            model=model_name,
            messages=[
                {"role": "system", "content": "You are an AI assistant providing diet recommendations for dental conditions."},
                {"role": "user", "content": f"Give food recommendations (do's and don'ts) for a person suffering from {predicted_condition}."}
            ]
        )
        recommendation = response["choices"][0]["message"]["content"]
    except openai.InvalidRequestError:
        # If GPT-4 is unavailable, use GPT-3.5
    ...
    return render_template('food.html', recommendation=recommendation)
